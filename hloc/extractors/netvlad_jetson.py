...
class NetVLADLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Same NetVLAD residual sum as the standard extractor, but without the
        # large BxDxKxN intermediate tensor. This is important on Jetson Nano.
        desc = torch.bmm(x, scores.transpose(1, 2))
        desc = desc - self.centers.unsqueeze(0) * scores.sum(dim=-1).unsqueeze(1)
        if self.intranorm:
...
class NetVLAD(BaseModel):
    ...
    def _init(self, conf):
        ...
        if conf["dtype"] not in {"float32", "float16"}:
            raise ValueError(f'Unsupported NetVLAD dtype: {conf["dtype"]}')
        weight_dtype = torch.float16 if conf["dtype"] == "float16" else torch.float32
        ...
        for layer, mat_layer in zip(self.backbone.children(), mat["net"].layers):
            if isinstance(layer, nn.Conv2d):
                ...
                w = torch.tensor(w, dtype=weight_dtype).permute([3, 2, 0, 1])
                b = torch.tensor(b, dtype=weight_dtype)
        ...
        score_w = torch.tensor(score_w, dtype=weight_dtype).permute([1, 0]).unsqueeze(-1)
        center_w = torch.tensor(center_w, dtype=weight_dtype)
        # Update layer weights.
        ...
        if conf["whiten"]:
            ...
            # Prepare for PyTorch - make sure it is float32 and has right shape
            w = torch.tensor(w, dtype=weight_dtype).squeeze().permute([1, 0])  # OUT x IN
            b = torch.tensor(b.squeeze(), dtype=weight_dtype)  # Shape: OUT
            # Update layer weights.
    ...
    def _forward(self, data):
        ...
        image = image.to(dtype=self.backbone[0].weight.dtype)
