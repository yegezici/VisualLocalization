...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Collect synchronized 4-camera CARLA data for HLoc/COLMAP')
    ...
    parser.add_argument('--fixed-delta', type=float, default=0.02,
                        help='Fixed simulation delta for synchronous mode')
    parser.add_argument('--timeout', type=float, default=2.0,
                        help='Sensor queue timeout in seconds')
    parser.add_argument('--camera-rig', default='cardinal', choices=['diagonal-corners', 'cardinal'],
                        help='Camera placement strategy (default: diagonal-corners)')
...
def retrieve_frame(sensor_queue: queue.Queue, target_frame: int, timeout: float):
    while True:
...
def main() -> None:
    ...
    sensor_queues: Dict[str, queue.Queue] = {}
    ...
    try:
        ...
        pygame.init()
        pygame.font.init()
        if len(active_camera_names) == 2:
            display = pygame.display.set_mode((width * 2, height), pygame.HWSURFACE | pygame.DOUBLEBUF)
        else:
            display = pygame.display.set_mode((width * 2, height * 2), pygame.HWSURFACE | pygame.DOUBLEBUF)
        pygame.display.set_caption('CARLA 4-Camera HLoc Capture')
        ...
        print(f'Controls: W/A/S/D to drive, SPACE to capture synchronized {len(active_camera_names)}-camera set, Q or ESC to quit.')
        ...
        while running:
            ...
            frame_images = {}
            for name in active_camera_names:
                frame_images[name] = retrieve_frame(sensor_queues[name], world_frame, args.timeout)

            if not all(frame_images[name].frame == world_frame for name in active_camera_names):
                print(f'[WARN] Frame mismatch at world_frame={world_frame}; skipping render/capture for this tick.')
                continue

            rgb_arrays = {name: image_to_rgb_array(frame_images[name]) for name in active_camera_names}
            ...
            for name, (ox, oy) in tile_order:
                surface = pygame.surfarray.make_surface(rgb_arrays[name].swapaxes(0, 1))
                display.blit(surface, (ox, oy))

            hud_text = f'Frame: {world_frame} | Captures: {capture_id} | SPACE: capture set'
            ...
            for camera_index, name in enumerate(active_camera_names, start=1):
                ...
                actor_snapshot = snapshot.find(sensors[name].id)
                if actor_snapshot is not None:
                    camera_tf = actor_snapshot.get_transform()
                else:
                    camera_tf = sensors[name].get_transform()
            ...
            print(f'[CAPTURE] id={capture_id} frame={world_frame} saved {len(active_camera_names)} images + poses')
            capture_requested = False
