...
try:
    ...
    from pygame.locals import K_l  # type: ignore[import-not-found]
...
def make_camera_transforms(rig_mode: str) -> Dict[str, carla.Transform]:
    ...
    if rig_mode == 'single':
        return {
            'front': carla.Transform(carla.Location(x=1.6, y=0.0, z=z_height), carla.Rotation(pitch=-4.0, yaw=0.0)),
        }
...
def location_error_m(estimated: carla.Location, ground_truth: carla.Location) -> float:
    dx = float(estimated.x) - float(ground_truth.x)
    dy = float(estimated.y) - float(ground_truth.y)
    dz = float(estimated.z) - float(ground_truth.z)
...
def recvall(sock: socket.socket, size: int) -> bytes:
    ...
    while remaining:
        ...
        if not chunk:
            raise ConnectionError('Localization server disconnected before sending the full response.')
        chunks.append(chunk)
        remaining -= len(chunk)
    return b''.join(chunks)


def request_visual_localization(
    rgb_array: np.ndarray,
    carla_map: carla.Map,
    gt_transform: carla.Transform,
    host: str,
    port: int,
    timeout: float,
    ...
    try:
        ...
        payload = rgb.tobytes()
        header = struct.pack('!III', int(width), int(height), len(payload))

        with socket.create_connection((host, port), timeout=timeout) as sock:
            sock.settimeout(timeout)
            sock.sendall(header + payload)
            response_size = struct.unpack('!I', recvall(sock, 4))[0]
            response = json.loads(recvall(sock, response_size).decode('utf-8'))

        if not response.get('success'):
            print(f'[Localization Failed] {response.get("error", "unknown error")}')
            return
        ...
        gt_location = gt_transform.location
        gt_geo = carla_map.transform_to_geolocation(gt_location)
        estimated_heading = float(response['heading_deg'])
        gt_heading = float(gt_transform.rotation.yaw)
        position_error = location_error_m(location, gt_location)
        heading_error = abs(wrap_angle_deg(estimated_heading - gt_heading))

        print(
            '[Localization Result] '
            f'lat={geo.latitude:.8f} lon={geo.longitude:.8f} alt={geo.altitude:.3f} '
            f'heading={estimated_heading:.2f}deg '
            f'x={location.x:.3f} y={location.y:.3f} z={location.z:.3f} '
            f'inliers={int(response.get("num_inliers", 0))}'
        )
        print(
            '[Localization Ground Truth] '
            f'lat={gt_geo.latitude:.8f} lon={gt_geo.longitude:.8f} alt={gt_geo.altitude:.3f} '
            f'heading={gt_heading:.2f}deg '
            f'x={gt_location.x:.3f} y={gt_location.y:.3f} z={gt_location.z:.3f}'
        )
        print(
            '[Localization Error] '
            f'position={position_error:.3f}m heading={heading_error:.2f}deg '
            f'dx={location.x - gt_location.x:.3f} '
...
def main() -> None:
    ...
    try:
        ...
        def submit_localization(image: carla.Image) -> None:
            with localization_lock:
                if localization_running[0]:
                    print('[Localization Busy] Previous localization request is still running.')
                    return
            ...
            if sensor is None:
                print(f'[Localization Skipped] No sensor found for camera {args.preview_camera}')
                with localization_lock:
                    localization_running[0] = False
                return
            gt_transform = get_image_sensor_transform(image, sensor)
